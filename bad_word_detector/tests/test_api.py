# bad_word_detector/tests/test_api.py
import sys
...
def test_detect_endpoint_with_non_toxic_text(client):
    ...
    response = client.post(
        "/detect", json={"text": "Hello, how are you today?", "threshold": 0.5}
    )
...
def test_detect_endpoint_with_toxic_text(client):
    ...
    response = client.post(
        "/detect", json={"text": "You are a stupid idiot", "threshold": 0.5}
    )
...
def test_detect_endpoint_with_different_languages(client):
    ...
    for expected_lang, text in languages.items():
        response = client.post("/detect", json={"text": text, "threshold": 0.5})
...
def test_detect_batch_endpoint(client):
    ...
    response = client.post("/detect_batch", json=texts)
...
def test_invalid_input(client):
    ...
    # Empty text
    response = client.post("/detect", json={"text": "", "threshold": 0.5})

    assert response.status_code == 200

    # Invalid threshold
    response = client.post("/detect", json={"text": "Hello", "threshold": 2.0})

    assert response.status_code == 200

    # Very long text
    long_text = "a" * 10000
    response = client.post("/detect", json={"text": long_text, "threshold": 0.5})
