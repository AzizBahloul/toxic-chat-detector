#!/usr/bin/env python
"""
Evaluation script for the bad word detection model.
"""
import os
import sys
import argparse
...
def parse_args():
    ...
    parser.add_argument(
        "--data_path", 
        type=str, 
        help="Path to evaluation data file (CSV/TSV)"
    )
...
def visualize_results(results, raw_predictions, test_labels, output_dir):
    ...
    sns.histplot(raw_predictions[test_labels == 0], 
                 color='green', alpha=0.5, bins=20, 
                 label='Non-Toxic', kde=True)
    sns.histplot(raw_predictions[test_labels == 1], 
                 color='red', alpha=0.5, bins=20, 
                 label='Toxic', kde=True)
    
    plt.axvline(results.get("threshold", 0.5), color='black', linestyle='--', 
...
def main():
    ...
    # Evaluate model
    results, raw_predictions = evaluate_model(model, test_texts, test_labels, threshold=args.threshold)
