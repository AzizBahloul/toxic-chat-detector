# bad_word_detector/scripts/download_dataset.py
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Download and prepare multilingual hate speech dataset"
    )
        ...
        if split_files:
            ...
            for config_name, dataset in dataset_dict.items():
                ...
                for split_name, split_data in dataset.items():
                    ...
                    logger.info(
                        f"Saved {config_name}/{split_name} with {len(df)} examples to {file_path}"
                    )
        else:
            ...
            if all_data:
                ...
                logger.info(
                    f"Saved combined dataset with {len(combined_df)} examples to {file_path}"
                )
            else:
